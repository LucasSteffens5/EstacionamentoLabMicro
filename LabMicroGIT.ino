...
#include <Servo.h>  // Biblioteca Utilizada oara controlar servo motor

/*------------------------ DEFINES ------------------------*/

#define RST_PIN    D3    // Define o pino reset do leitor RF
#define SS_PIN     D8   // Define o pino SS do leitor RF
...
const int PinoGatilho = D0;  //D0  Pinos de trigger e echo do sonar
const int PinoEcho = D2;  //D2
...
long duracao; // Variavel auxiliar que ajuda no periodo de avaliação do sonar
int disntacia; // variavel para representar distancia do sonar
...
void setup()
{
pinMode(PinoGatilho, OUTPUT); //PinoGatilho como saida
pinMode(PinoEcho, INPUT); //PinoEcho como entrada  

  
  ESP.wdtDisable(); //Desabilita o WDT de software (conta até 3,5 s) E consequentemente habilita o de hardware (hardware vai ate 8s)
  Serial.begin(9600);
...
//int d = distancia(); // Reaizado para testes do sonar
//Serial.println(d);
  ...
  {
    // Serial.print(mfrc522.uid.uidByte[i] < 0x10 ? " 0" : " ");  // Printa UID do cartao rfid // realizado para testes dos cartoes e leitor
  ...
  conteudo.toUpperCase();
   ...
   if((conteudo.substring(1)=="33 16 53 73")&&(usuario1==false)){ //UID 2 - Cartao chaveiro campus
    ...
    Serial.println("Seja bem-vindo Yuri !");
    Serial.println();
     cancelaSobe();
     int d = distancia();
  while(d<=10){ // Enquanto a distancia for menor que 10 cm a cancela fica aberta
    
    d=distancia();
   //Serial.println(d); // Utilizado para testes
   delay(100);
    }
  ...
  while(d<=10){
   ...
   delay(100);
    ...
    Serial.println("Seja bem-vindo Lucas Steffens ! ");
    Serial.println();
     cancelaSobe();
 ...
 Serial.print(" Volte sempre, será enviado um e-mail com o recibo do valor cobrado de seu cartao de crédito. ");
  ...
  ESP.wdtFeed();  // Manda o callback pro WDT de hardware não reiniciar
...
void enviaEmail(String email, String nome){
  ...
  SMTP.setEmail("*********@gmail.com") // Email que ira mandar o e-mail  // Com as configurações SMTP realizadas no e-mail
    .setPassword("*********")  // Senha do e-mail que ira mandar o e-mail
...
int distancia(){
  // Limpa o pino Trigger PinoGatilho
digitalWrite(PinoGatilho, LOW);
delayMicroseconds(2);

// Seta Trigger para alto e depois para baixo 
digitalWrite(PinoGatilho, HIGH);
delayMicroseconds(10);
digitalWrite(PinoGatilho, LOW);

// Le o Pino ECHO que pega informação do sonar
duracao = pulseIn(PinoEcho, HIGH);

// Transforma em CM -- Calculo retirado da biblioteca
disntacia= duracao*0.034/2;
